...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlOnPolicyRunnerCfg):
    ...
    agent.load_net_weights(path=fullpath,filename="weight_2000")
    ...
    while simulation_app.is_running():
        ...
        with torch.inference_mode():
            ...
            for episode in range(n_episodes):
                ...
                while not done:
                    # agent stepping
                    prob_each_action = agent.policy_net(obs['policy']) # > tensor([[0.1380, 0.1534, 0.1328, 0.1328, 0.1656, 0.1328, 0.1446]],device='cuda:0', grad_fn=<SoftmaxBackward0>)
                    ...
                    # env stepping
                    next_obs, reward, terminated, truncated, _ = env.step(agent.scale_action(action_idx))
